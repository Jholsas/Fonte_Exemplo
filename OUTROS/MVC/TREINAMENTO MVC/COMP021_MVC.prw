...
oModel:AddGrid( 'ZA2DETAIL', 'ZA1MASTER', oStruZA2, /*bLinePre*/, /*bLinePost*/, {|oModelGrid|tstPreValid(oModelGrid)}/*bPreVal*/, /*bPosVal*/, /*BLoad*/ )
...
oView:SetViewProperty('VIEW_ZA2', "ENABLENEWGRID")
...
//oView:SetViewProperty( 'VIEW_ZA2', "CHANGELINE", {{ |oView, cViewID| ChangeLine(oView, cViewID) }}  )
...
//oView:AVIEWS[2][3]:BDOUBLECLICK := {|| Alert("Duploclick")}
...
If nOperation == MODEL_OPERATION_INSERT .OR. nOperation == MODEL_OPERATION_UPDATE
	...
	For nI := 1 To  oModelZA2:Length()
		...
		If !oModelZA2:IsDeleted()
			If ZA0->( MsSeek( xFilial( 'ZA0' ) + oModelZA2:GetValue( 'ZA2_AUTOR' ) ) ) .AND. ZA0->ZA0_TIPO == '1'
				lAchou := .T.
				Exit
			EndIf
		EndIf
...
ElseIf nOperation == MODEL_OPERATION_UPDATE
	...
	ZA0->( dbSetOrder( 1 ) )
	If ZA0->( MsSeek( xFilial( 'ZA0' ) + FwFldGet( 'ZA2_AUTOR' ) ) )
...
Static Function MyDoubleClick()

Alert('MyDoubleClick')

Return .T.

Static Function tstPreValid(oModelGrid)
...
if nOperation == MODEL_OPERATION_INSERT .or. nOperation == MODEL_OPERATION_UPDATE
	//oModel:LoadValue( 'ZA2DETAIL', 'ZA2_AUTOR', '000001' )
	//oModelGrid:aCols[1][7] := '00'
	//oModelZA2:LoadValue('ZA2_LOCAL' , '01' )
	//oModel:AMODELSTRUCT[1][3]:ADATAMODEL[1][7][2] := '00'
EndIf
